    ...
    if model_family == "resnet":
        ...
        if with_transformer == True:
            if translation_model == "fc":
                model = stn_resnet.Model(config.model,num_ids)
                print("Using stn_resnet")
            if translation_model == "conv":
                model = stn_resnet_conv.Model(config.model,num_ids)
                print("Using stn_resnet_conv")
            if translation_model == "convLarge":
                model = stn_resnet_conv_large.Model(config.model,num_ids)
                print("Using stn_resnet_conv_large")
        else:
...
#     train_step = optimizer.minimize(total_loss, global_step=global_step)
    ...
    # tf.summary.scalar('nat_reg_loss', model.reg_loss, collections=['nat'])
    ...
    with tf.Session() as sess:
        ...
        for ii in range(max_num_training_steps+1):
            ...
            ########### Check the differences between theta and transformation para #############
            print("Print First 5 samples' predicted adv theta")
            theta_temp = sess.run(model.theta, feed_dict=adv_dict)
            print(theta_temp[0:5])
            print("Print First 5 samples' rel_trans")
            trans_temp = sess.run(model.transform, feed_dict=adv_dict)
            print(trans_temp[0:5])
            # print("Print First 5 samples' check remain")
            # trans_temp = sess.run(model.transform, feed_dict=adv_dict)
            # print(theta_temp[0:5] - trans_temp[0:5])
            ...
            if ii % num_output_steps == 0:
                ...
                # reg_loss = sess.run(model.reg_loss, feed_dict=adv_dict)
                # print('    training adv reg_loss {:.4}%'.format(reg_loss))
            ...
            if ii % num_summary_steps == 0:
                ...
                # summary = sess.run(eval_summaries, feed_dict=eval_dict)
