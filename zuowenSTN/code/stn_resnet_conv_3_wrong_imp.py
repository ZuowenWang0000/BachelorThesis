...
class Model(object):
  ...
  def __init__(self, config, num_ids):
    """ResNet constructor.
    """
    self._build_model(config,config.filters, num_ids,
  ...
  def _build_model(self, config, filters, num_ids, pad_mode='CONSTANT', pad_size=32):
    ...
    with tf.variable_scope('logit'):
      self.pre_softmax = self._fully_connected('logits_', x, config.n_classes)
  ...
  def _stn(self, x, H, W, C, pad_mode):
    ...
    with tf.variable_scope('spatial_transformer'):
      ...
      x_tensor = tf.reshape(x, [-1, H, W, C])
      with tf.variable_scope('locnet'):
        conv_trans = self._batch_norm('bn_loc', x)
        #                                                     32
        conv_trans = self._conv('conv_loc1', conv_trans, 3, 3, 32, [1, 1, 1, 1])
        conv_trans = self._relu(conv_trans, 0.1)
        conv_trans = self._maxpool2d(conv_trans)
        # conv_trans = tf.layers.dropout(conv_trans, 0.25)
        #                                                   64  128
        conv_trans = self._conv('conv_loc2', conv_trans, 3, 32, 64, [1, 1, 1, 1])
        conv_trans = self._relu(conv_trans, 0.1)
        conv_trans = self._maxpool2d(conv_trans)
        # conv_trans = tf.layers.dropout(conv_trans, 0.25)
        #                                                   128
        conv_trans = self._conv('conv_loc3', conv_trans, 3, 64, 128, [1, 1, 1, 1])
        conv_trans = self._relu(conv_trans, 0.1)
        conv_trans = self._maxpool2d(conv_trans)
        conv_trans = self._fully_connected('fc1', conv_trans, 128)

        # conv_trans = tf.layers.dropout(conv_trans, 0.25)

        conv_trans = self._relu(conv_trans, 0.1)

        #conv_trans(equivalent to theta) already initialized to identity?
        conv_trans = self._fully_connected_iden('fc2', conv_trans, 3)
        conv_trans = self._relu(conv_trans)
        self.theta = conv_trans
        self.trans_x, self.trans_y, self.rot = tf.unstack(self.theta, axis=1)
        self.reg_loss = tf.reduce_sum(tf.losses.mean_squared_error(
          self.theta,
          self.transform)
      )

      # x : input_feature_map: the output of the layer preceding the localization network.
      # If the STN layer is the first layer of the network, then this corresponds to the
      # input images. Shape should be (B, H, W, C).

      # conv_trans: theta: this is the output of the localization network. Shape should be (B, 6)

      h_trans = self.transformer(x_tensor, conv_trans, out_dim, pad_mode)
  ...
  # last fully connected layer, outputing 0 rot, 0 translation at beginning
  def _fully_connected_iden(self, name, x, out_dim):
    """FullyConnected layer for final output, intialized as identity (bias)"""
    with tf.variable_scope(name):
      num_non_batch_dimensions = len(x.shape)
      prod_non_batch_dimensions = 1
      for ii in range(num_non_batch_dimensions - 1):
      ...
      b = tf.Variable(initial_value=initial, name ='biases')
      return tf.nn.xw_plus_b(x, w, b)
