...
class Model(object):
  ...
  def _build_model(self, config, filters, num_ids, differentiable=False,
    ...
    with tf.variable_scope('input'):
      ...
      gconv_indices, gconv_shape_info, w_shape = gconv2d_util(
        h_input='Z2', h_output='D4', in_channels=3, out_channels=filters[0], ksize=3)

      w = tf.Variable(tf.truncated_normal(w_shape, stddev=1.))
      x = gconv2d(input=x, filter=w, strides=[1, 1, 1, 1], padding='SAME',
  ...
  def _residual(self, x, in_filter, out_filter, stride,
    ...
    with tf.variable_scope('sub1'):
      x = self._gconv('conv1', x, 3, in_filter, out_filter, stride)

    with tf.variable_scope('sub2'):
      x = self._batch_norm('bn2', x)
      x = self._relu(x, 0.1)
      x = self._gconv('conv2', x, 3, out_filter, out_filter, [1, 1, 1, 1])

    with tf.variable_scope('sub_add'):
      if in_filter != out_filter:
        # bc the real filter number in 2D = 3D filter number *4 (C4)
        rel_out = out_filter*8
        rel_in = in_filter*8
        ...
        orig_x = tf.pad(
            orig_x, [[0, 0], [0, 0], [0, 0],
                     [(rel_out-rel_in)//2, (rel_out-rel_in)//2]])
      x += orig_x
  ...
  def _gconv(self, name, x, filter_size, in_filters, out_filters, strides):
    """G - Convolution."""
    gconv_indices, gconv_shape_info, w_shape = gconv2d_util(
      h_input='D4', h_output='D4', in_channels=in_filters, out_channels=out_filters, ksize=3)
    w = tf.Variable(tf.truncated_normal(w_shape, stddev=1.))
    x = gconv2d(input=x, filter=w, strides=strides, padding='SAME',
                gconv_indices=gconv_indices, gconv_shape_info=gconv_shape_info, name=name)
    return x
