               ...
               # first num_id examples of batch are natural
                ...
                for _ in range(group_size-1):
                    ...
                    x_batch_inp = np.concatenate(
                        (x_batch_inp, x_batch_adv_i), axis=0)
                    y_batch_inp = np.concatenate(
                        (y_batch_inp, y_batch), axis=0)
                    trans_inp = np.concatenate(
                        (trans_inp, trans_adv_i), axis=0)
                    id_batch_inp = np.concatenate(
                        (id_batch_inp, id_batch), axis=0)
                end = timer()
