...
def train(config='configs/cifar10_nat_res.json',
    ...
    if model_family == "resnet":
        ...
        model = resnet.Model(config.model, num_ids, diffable,
            config.training.adversarial_ce, config.training.nat_ce)
    ...
    with tf.Session() as sess:
        ...
        for ii in range(max_num_training_steps+1):
            ...
            if ii % num_output_steps == 0:
                ...
                core_loss = sess.run(model.core_loss2, feed_dict=inp_dict)
                ...
                # print('    training inp trades loss {:.4}'.format(core_loss))
