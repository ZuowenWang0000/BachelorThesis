...
import resnet_reg_stand
import vgg
from spatial_attack_stand import SpatialAttack
import utilities
import pickle
...
def train(config='configs/cifar10_regression.json',
    ...
    if model_family == "resnet":
        if config.attack.use_spatial and config.attack.spatial_method == 'fo':
            ...
            diffable = False
        model = resnet_reg_stand.Model(config.model, num_ids, diffable,
    ...
    tf.summary.scalar('regression loss function value', model.reg_loss)
    ...
    with tf.Session() as sess:
        ...
        if simple_train:
            ...
            transform_parameters = attack.simple_train_perturb(x_eval_batch)
            x_batch_eval = x_eval_batch
            y_batch_eval = transform_parameters
            trans_eval = transform_parameters
        ...
        for ii in range(max_num_training_steps+1):
            ...
            if use_core:
                ...
                if adversarial_training:
                    ...
                    if simple_train:
                        attack = SpatialAttack(model, config.attack, config.attack.spatial_method,
                                               ...
                                               fo_step_size, fo_num_steps)
                        # attack.simple_train_perturb should return a list of parameters (len(x_batch),3)
                        transform_parameters = attack.simple_train_perturb(x_batch)
                        x_batch_inp = x_batch
                        trans_inp = transform_parameters

                        # if ii % num_easyeval_steps == 0 or ii == max_num_training_steps:
                        #     print("in train_reg")
                        #     print(transform_parameters)
                        # utilities.show_image_batch(x_batch_inp, 2)
                    else:
                ...
                if simple_train:
                    y_batch_inp = transform_parameters
            ...
            if epoch_done:
                ...
                if ii == max_num_training_steps:
                    print_eval_fullbatch(sess, y_batch_eval, batch_size, trans_eval, model)
            ...
            # if ii % num_easyeval_steps == 0 or ii == max_num_training_steps:
            #     # create a checkpoint
            #     print("CREATING SIMPLE CHECKPOINT\n")
            #     chkpt_id = tf.train.Checkpoint()
                ...
                #     training_step=ii, epoch=no_epochs_done, reg_loss=loss,
                #     )
...
def print_eval_fullbatch(sess, y_batch_eval, batch_size, trans_eval, model):
    features, label = load_cfar10_test_batch("../code/scratch/datasets/cifar10/test_batch")

    eval_dict = {model.x_input: features,
                 model.y_input: y_batch_eval,
                 # group is not used in simple train
                 model.group: np.arange(0, batch_size, 1, dtype="int32"),
                 model.transform: trans_eval,
    ...
    eval_avg_abs_err_transX = sess.run(model.avg_abs_err_transX, feed_dict=eval_dict)
    eval_avg_abs_err_transY = sess.run(model.avg_abs_err_transY, feed_dict=eval_dict)
    eval_avg_abs_err_rot = sess.run(model.avg_abs_err_rot, feed_dict=eval_dict)
    eval_avg_rel_err_transX = sess.run(model.avg_rel_err_transX, feed_dict=eval_dict)
    eval_avg_rel_err_transY = sess.run(model.avg_rel_err_transY, feed_dict=eval_dict)
    eval_avg_rel_err_rot = sess.run(model.avg_rel_err_rot, feed_dict=eval_dict)

    print('avg_abs_err_transX : {}'.format(eval_avg_abs_err_transX))
    print('avg_abs_err_transY : {}'.format(eval_avg_abs_err_transY))
    print('avg_abs_err_rot : {}'.format(eval_avg_abs_err_rot))
    print('avg_rel_err_transX : {}'.format(eval_avg_rel_err_transX))
    print('avg_rel_err_transY : {}'.format(eval_avg_rel_err_transY))
    print('avg_rel_err_rot : {}'.format(eval_avg_rel_err_rot))
...
@click.command()
@click.option('--config', default='configs/fannyconfig.json', type=str)
