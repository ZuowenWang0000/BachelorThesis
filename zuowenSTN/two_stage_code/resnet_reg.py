...
class Model(object):
  ...
  def _build_model(self, config, filters, num_ids, differentiable=False,
    ...
    with tf.variable_scope('input'):
      ...
      self.x_input = tf.placeholder(tf.float32, shape=[None, 32, 32, 3], name="x_input")
      #MODIFIED now y_input is a three dimensional label

      self.y_input = tf.placeholder(tf.float32, shape=[None, config.n_regression_labels], name="y_input") #should be 3

      self.transform = tf.placeholder(tf.float32, shape=[None, 3], name="transform")
      ...
      if not differentiable:
        ...
        print("in training localization net, this section should be entered, go check error\n")
    ...
    print("\nin resnet_reg")
    # print("y_input")
    # tf.Print(self.y_input)
    #
    # print("\nprediction")
    # tf.Print(self.prediction)
    err = tf.abs(self.prediction - self.y_input)

    with tf.variable_scope('avg_abs_error'):
      self.avg_abs_err_transX = tf.reduce_mean(err[:1])
      self.avg_abs_err_transY = tf.reduce_mean(err[:2])
      self.avg_abs_err_rot = tf.reduce_mean(err[:3])
    with tf.variable_scope('avg_rel_error'):
      self.avg_rel_err_transX = tf.abs(tf.reduce_mean(err[:1]/self.y_input[:1]))
      self.avg_rel_err_transY = tf.abs(tf.reduce_mean(err[:2]/self.y_input[:2]))
      self.avg_rel_err_rot = tf.abs(tf.reduce_mean(err[:3]/self.y_input[:3]))


    with tf.variable_scope('costs'):
      # if adversarial_ce:
      #  print("WARNING, should not be entering here for regression task\n")
       # indices_adv = tf.cast(
      ...
      # self.y_abs_error = tf.abs(self.prediction - self.y_input)
      # self.mean_abs_error = tf.map_fn(lambda )
      # los = tf.losses.absolute_difference(self.y_input, self.prediction)
      #TODO (Zuowen)  not sure this loss is correct&/suitable yet.
